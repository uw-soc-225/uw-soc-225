...
You can see that they're joined together with no space in between. If you want to change that, use `sep`:
...
The basic idea is that we take a **string** (or vector of strings) and apply some sort of **pattern** to it. There are different kinds of things we might want to get back out of this. For example: 

- only the entries that match the pattern (`str_subset`)
- TRUE or FALSE (`str_detect`)
- the pattern itself (`str_extract`)
...
data_frame(fruit) %>%
  mutate(first_letter = str_sub(fruit, 1, 1)) %>%
  ggplot(aes(x = fct_rev(fct_infreq(first_letter)))) + 
  geom_bar() + 
  coord_flip() + 
  ylim(0, 13)
...
data_frame(fruit) %>%
  mutate(first_letter = str_sub(fruit, 1, 1)) %>%
  ggplot(aes(x = fct_rev(first_letter))) + 
  geom_bar() + 
  coord_flip() + 
  ylim(0, 13)
```
...
Let's say we're interested in trends in fruit names.
...
str_split(best_coast, ", ")
str_split_fixed(best_coast, ", ", n = 2)
