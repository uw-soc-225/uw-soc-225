...
For vectors, matrices, and arrays, accessing the data you store in them is as simple as []. That is, you just need to throw brackets around the location of the value you want and you'll get it. Each value in one of these data types has a kind of 'address' made up of a numeric vector with lenght equal to the number of dimensions, starting with rows, then columns, then height, then... whatever the fourth demension is called. 
...
tree_heights <- c(19, 20, 35, 5) # 100% real measured tree heights that I made with a measuring tape (not really)
