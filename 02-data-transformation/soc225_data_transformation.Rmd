...
bread %>% Cut() %>% Smear(condiment = 'peanut_butter') %>% Apply(what = 'pickles', method = 'carefully') %>% close()
...
Unfortunetly, we don't have the technology yet to make real sandwiches this way. Instead, we'll be doing the same thing to data.
...
```{r}
# instead of
head(unique(airbnb_data$neighbourhood)) # show the top sice unique neighborhoods

# we can write
airbnb_data$neighbourhood %>% unique() %>% head() # show the top sice unique neighborhoods
...
This is how we generated the data for Week 1.
...
airbnb_data %>% # take the airbnb data
  ...
  filter(price > 0) %>% # exclude negative prices
  arrange(price) # arrange by acsending price order
```
