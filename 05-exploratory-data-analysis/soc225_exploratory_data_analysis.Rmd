...
Unfortunetly, that's not the only column that we need to change. Fortunetly mutate has a sibling function called `mutate_at` which lets us specify groups of columns and apply a function to them.

** Question 3.4 Use mutate_at to change all the columns that start with 'Percent' so that they're numberic.**
...
schools %>% # take our modified data
  ggplot(aes(x = `Percent of Students Chronically Absent`, # assignt the x and y aestetics
                    y = `Student Attendance Rate`)) + 
  geom_point(alpha = .5) + # make a scatterplot, increasing transparency to show when points fall in the same place
...
schools %>% # take the schools data
  ggplot(aes(x = `Economic Need Index`, y = `Percent White`)) + # grab our variables
...
```{r}
schools %>% # take the schools data
  gather(key = "Race", value = "Percent", # use gather to make long data
         `Percent Asian`, `Percent Black`, 
         `Percent Hispanic`, `Percent White`) %>%
  mutate(Percent = Percent/100) %>% # make nicer percents by makeing them proportions
  ggplot(aes(x = `Economic Need Index`, y = Percent, color = Race)) + # start our plot, coloring by race
  geom_point(alpha = .5) + # increase transparancy
  geom_smooth(method = "lm", color = "black") + # specify linear smoothing
  facet_wrap(~Race) + # wrap by Race
  guides(color = FALSE) + # get rid of the color legend
  scale_y_continuous(labels = scales::percent) + # make a percent scale
  labs(title = "Student racial composition and economic need") # add a title
```


** Question 3.11: Interpret our results **
...
** Question 4.2 Explore the variables in the data you picked. Which variables seem like they might address your interest? What mode of data are they in? Make a plot for each showing its varience. **
...
** Question 4.4 Investigate the covarience between your two variables with a scatterplot **  
...
4.2 These will be bar plots if your data is catagorical, histograms if its continuous. If the data is categorical but has lots of different values, you should consider combining it using factors.