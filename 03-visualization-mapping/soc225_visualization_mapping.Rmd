...
In addition to the x and y axes, another aesthetic available to use is `color`. This meas we can look at how up to three variables change together!
...
### 3.1 Create a plot where `x = year` and `y = lifeExp`. This time, use a new geom: `geom_line()` instead of `geom_point()`. 

### 3.2 Try creating a line plot of life expectancy by year. Initially, it won't look quite right.  

### 3.3 You need to tell the plot to group the lines by country. To do this, you'll need a new aesthetic, `group = country`. Incorporate this into your plot. Does it look more reasonable now?

### 3.4 Finally, facet by continent. Does life expectancy seem to have increased over time everywhere? Do you see any dips or decreases? 
...
**Question 4.1 Make a leaflet plot that only includes only listings with a price over $200 and shows the price when the circle is clicked**
...
example_data <- airbnb_data %>% # assign the small dataframe the name 'example_data'
...
**Question 4.1: Use the same code from above, but map a neighborhood group of your choice**
...
### 5.1 Think about what varible you'd like to display in your map.
...
### 5.2 Create a bar plot or histogram as appropriate to look at the distribution of your variable
